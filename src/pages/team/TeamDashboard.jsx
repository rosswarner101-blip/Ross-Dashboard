...
import { Plus, Search, User, Building2, Upload, Loader2, Trash2 } from 'lucide-react';
...
const TeamDashboard = () => {
    ...
    const handleDeletePendingClient = async (clientId) => {
        if (!confirm('Are you sure you want to delete this pending client? This action cannot be undone.')) return;
        ...
        try {
            ...
            // Update local state
            setPendingClients(pendingClients.filter(c => c.id !== clientId));
            alert('Pending client deleted successfully.');
        } catch (error) {
            console.error('Error deleting pending client:', error);
            alert(`Failed to delete pending client: ${error.message}`);
        }
    };
                                        ...
                                        <tr key={c.id}>
                                            ...
                                            <td className="px-6 py-4 text-sm text-right">
                                                <div className="flex items-center justify-end gap-2">
                                                    ...
                                                    <button
                                                        onClick={() => handleDeleteActiveClient(c.id)}
                                                        className="text-red-500 hover:text-red-700 p-1 hover:bg-red-50 rounded transition-colors"
                                                        title="Delete Client"
                                                    >
                                                        <Trash2 size={16} />
                                    ...
                                    {pendingClients.map((c) => (
                                        <tr key={c.id}>
                                            ...
                                            <td className="px-6 py-4 text-sm text-right flex items-center justify-end gap-2">
                                                ...
                                                <button
                                                    onClick={() => handleDeletePendingClient(c.id)}
                                                    className="text-red-500 hover:text-red-700 p-1 hover:bg-red-50 rounded transition-colors"
                                                    title="Delete Pending Client"
                                                >
                                                    <Trash2 size={16} />
                                                </button>
                                            </td>
