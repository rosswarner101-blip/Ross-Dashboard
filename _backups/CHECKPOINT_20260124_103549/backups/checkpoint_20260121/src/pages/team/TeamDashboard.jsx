...
const TeamDashboard = () => {
    ...
    const handleClientSubmit = async (e) => {
        ...
        try {
            ...
            if (data) {
                // Update local state with returned data (lines up with the expected schema)
                setClients([data[0], ...clients]);
                alert('Client added successfully!');
                setClientFormData(initialClientFormState);
                setActiveTab('status'); // Auto-switch to status tab to show the result
            }
        } catch (error) {
            ...
            <div className="min-h-[500px]">
                {activeTab === 'addData' ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 flex flex-col md:col-span-2 lg:col-span-1 min-w-0">
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                            <div className="px-6 py-4 border-b border-slate-200 flex justify-between items-center bg-slate-50">
                                <h3 className="font-semibold text-slate-800 flex items-center gap-2">
                                    <Building2 size={18} className="text-slate-500" /> Client Status
                                </h3>
                            </div>
                            <table className="min-w-full divide-y divide-slate-200">
                                <thead className="bg-slate-50">
